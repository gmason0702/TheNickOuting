...
describe("zonedTimeToUtc", () => {
  ...
  it("crosses midnight UTC correctly for a late evening local time", () => {
    expect(zonedTimeToUtc("2026-10-02", "20:30", "America/Indiana/Indianapolis").toISOString()).toBe(
...
describe("golfCalendarEvent / receptionCalendarEvent", () => {
  it("golf runs 2:00pm-6:30pm local, reception 6:30pm-8:30pm local", () => {
    ...
    expect(golf.end.toISOString()).toBe("2026-10-02T22:30:00.000Z");
    expect(reception.start.toISOString()).toBe("2026-10-02T22:30:00.000Z");
    expect(reception.end.toISOString()).toBe("2026-10-03T00:30:00.000Z");
  });
});
...
describe("buildIcsCalendar", () => {
  it("emits a VCALENDAR with one VEVENT per event, using UTC timestamps and the venue location", () => {
    ...
    expect(ics).toContain("DTEND:20261002T223000Z");
...
describe("buildGoogleCalendarUrl", () => {
  it("builds a Google Calendar template link with UTC dates and the venue location", () => {
    ...
    expect(url.searchParams.get("dates")).toBe("20261002T180000Z/20261002T223000Z");
